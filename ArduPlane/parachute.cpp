...
void Plane::parachute_check()
{
    ...
    if (parachute.auto_enabled() &&
        ...
        if (abs(ahrs.pitch_sensor) >= PARACHUTE_CRITICAL_ANGLE_DEVIATION_PITCH) {
            parachute_release();
            if (critical_angle_msg_sended == false) {
                gcs().send_text(MAV_SEVERITY_CRITICAL, "Parachute released: Reached critical pitch angle");
                critical_angle_msg_sended = true;
            }
        }
        if (abs(ahrs.roll_sensor) >= PARACHUTE_CRITICAL_ANGLE_DEVIATION_ROLL) {
            parachute_release();
            if (critical_angle_msg_sended == false) {
                gcs().send_text(MAV_SEVERITY_CRITICAL, "Parachute released: Reached critical roll angle");
                critical_angle_msg_sended = true;
            }
        }
    }
    ...
    if (parachute.auto_enabled()) {
        ...
        if (alt_reached && !chute_auto_ready) {
